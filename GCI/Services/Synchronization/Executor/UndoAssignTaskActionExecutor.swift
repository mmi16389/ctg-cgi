//
//  UndoAssignTaskActionExecutor.swift
//  GCI
//
...
class UndoAssignTaskActionExecutor: TaskActionExecutor {
    
    let offlineEnabled = false
    let neededDataType = ActionDataType.none
    let successMessage = "task_action_undo_assign_success".localized
    ...
    func prefilledObject(fromTask task: TaskViewModel) -> Any? {
        guard task.canUndoAssign else {
            return nil
        }
        
        return ActionWorkflowViewModel(taskId: task.id, workflowAction: .undo, taskAction: .assign)
    }
